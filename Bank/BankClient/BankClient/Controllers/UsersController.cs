    ...
    public class UsersController : Controller
    {
        ...
        public ActionResult Create(Users obj)
        {
            try
            {
                ...
                RestClient restClientCheck = new RestClient();
                restClientCheck.BaseUrl = mst.BaseUrl;
                restClientCheck.endPoint = $"api/CheckUsername?userName={Uri.EscapeDataString(obj.UserName)}";
                string checkResult = restClientCheck.RestRequestAll();

                bool isExist = JsonConvert.DeserializeObject<bool>(checkResult);
                if (isExist)
                {
        ...
        public ActionResult Delete(int id, IFormCollection collection)
        {
            try
            {
                ...
                {
                    ViewBag.ErrorMessage = "Cannot remove user Admin.";
                    return View(); // hoặc trả về lại View hiện tại
                }
        ...
        public ActionResult Login(Users user, bool Remember)
        {
            ...
            Console.WriteLine($"Received: UserName={user.UserName}, Password={user.UserPass}, Remember={Remember}");
            ...
            if (true) // Bỏ ModelState.IsValid
            {
                ...
                RestClient restClient = new RestClient();
                restClient.BaseUrl = mst.BaseUrl;
                restClient.endPoint = $"api/Login?userName={Uri.EscapeDataString(user.UserName)}&password={Uri.EscapeDataString(user.UserPass)}";
                string result = restClient.RestRequestAll();
